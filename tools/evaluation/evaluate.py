...
def evaluate(config, config_path=None):
    ...
    # set non-tree label to -1 and tree nums to 0, 1, 2, etc. for the purpose of evaluation 为评估目的，将非树标签设为-1，树编号设为 0、1、2 等
    ...
    # propagate instance predictions to gt pointcloud coordinates via nearest neighbors so that they can be compared 通过近邻将实例预测传播到 gt 点云坐标，以便进行比较
    ...
    ################################## GETTING DETECTION RESULTS 获取检测结果
    # hungarian matching
    ...
    # get detection failures by comparing matched gts and preds to all gts and preds 通过将匹配的 gts 和 preds 与所有 gts 和 preds 进行比较，获取检测失败情况
    ...
    ################################## GETTING SEGMENTATION RESULTS 获取细分结果
    ...
    # 基于最大 iou 的唯一 gts 和相应 preds，以计算覆盖率（根据 ForAINet 论文）以及点级别的精确度和召回率
    ...
    # get fine-grained instance segmentation evaluation 获得精细的实例分割评估
    ...
    ################################## MAP RESULTS TO ORIGINAL LABELS AND CALCULATE METRICS 将结果映射到原始标签并计算指标
    ...
    # calculate aggregated segmentation metrics 计算汇总的细分指标
    ...
    # # 保存传播到 gt 点云的预测结果（与 gt 点云的点和点排序相同，可能对某些分析有用）
